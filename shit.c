...
/*   Updated: 2025/06/24 23:35:36 by root             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool tokenize(t_tokenizer *tokenizer, t_shell *shell)
{
    while (tokenizer->input[tokenizer->pos])
    {
        ...
        if (c == ' ')
        {
            token = create_token(TOKEN_SPACE, " ");
            tokenizer->pos++;
        }
        // Handle operators
        if (c == '|')
...
t_command *parse_tokens(t_token *tokens)
{
    ...
    while (tokens && tokens->type != TOKEN_EOF)
    {
                  ...
                  tokens->type == TOKEN_HEREDOC)
        {
            ...
            if (tokens->next)
                tokens = tokens->next;
        } else if (tokens->type == TOKEN_SINGLE_QUOTE || tokens->type == TOKEN_DOUBLE_QUOTE) {
            static char *current_arg = NULL;

            // Start buffer if null
            if (!current_arg)
                current_arg = ft_strdup("");

            // Append this quoted token
            if (tokens->value) {
                char *tmp = ft_strjoin(current_arg, tokens->value);
                free(current_arg);
                current_arg = tmp;
            }
            printf("Current argument: %s\n", current_arg);
            printf("token next type: %d\n", tokens->next->type);

            // If next token is NULL, space, or operator → finalize argument
            if (!tokens->next ||
                tokens->next->type == TOKEN_SPACE || tokens->next->type == TOKEN_EOF ||
                is_operator_token(tokens->next)) {
                printf("Finalizing argument: %s\n", current_arg);
                if (!current) {
                    current = malloc(sizeof(t_command));
                    if (!current) {
                        free(current_arg);
                ...
                if (!current->command)
                    current->command = strdup(current_arg);
                else
                    add_arg(current, current_arg);
                current_arg = NULL;
            }
        }
        tokens = tokens->next;
